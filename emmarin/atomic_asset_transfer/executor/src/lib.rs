...
impl ZoneNote {
    pub fn new_with_pks(
        zone_name: &str,
        authorized_pks: Vec<NullifierCommitment>,
        ledger: LedgerWitness,
        mut rng: impl CryptoRngCore,
    ) -> Self {
        let state = StateWitness { authorized_pks };
        let state_note = zone_state_utxo(&state, ledger, &mut rng);
        Self { state, state_note }
    }

    pub fn zone_input_witness(&self) -> cl::InputWitness {
        cl::InputWitness::public(self.zone_note)
    }
...
pub fn zone_stf_constraint() -> Constraint {
...
pub fn prove_zone_stf(
    ...
    ledger: LedgerWitness,
    txs: Vec<Tx>,
    tx_auths: Vec<risc0_zkvm::Receipt>,
    ...
    for tx_auth in tx_auths {
        env.add_assumption(tx_auth);
    }
    let env = env.write(&private_inputs).unwrap().build().unwrap();
    ...
    let prove_info = prover
        .prove_with_opts(env, emmarin_risc0_proofs::ZONE_STF_ELF, &opts)
        .unwrap();
    println!(
    ...
    ledger::ConstraintProof::from_risc0(emmarin_risc0_proofs::ZONE_STF_ID, receipt)
}
