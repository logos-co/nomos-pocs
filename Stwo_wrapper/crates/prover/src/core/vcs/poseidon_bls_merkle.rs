...
use crate::core::channel::{MerkleChannel, PoseidonBLSChannel};
...
pub struct PoseidonBLSMerkleHasher;
impl MerkleHasher for PoseidonBLSMerkleHasher {
    type Hash = BlsFr;
    ...
    fn hash_node(
        ...
        for chunk in padded_values.array_chunks::<ELEMENTS_IN_BLOCK>() {
            let mut word = BlsFr::default();
            for x in chunk {
                word = word * BlsFr::from(2u64.pow(31)) + BlsFr::from(x.0);
            }
            values.push(word);
        }
        poseidon_hash_many_bls(&values)
    }
}
...
#[derive(Default)]
pub struct PoseidonBLSMerkleChannel;

impl MerkleChannel for PoseidonBLSMerkleChannel {
    type C = PoseidonBLSChannel;
    type H = PoseidonBLSMerkleHasher;

    fn mix_root(channel: &mut Self::C, root: <Self::H as MerkleHasher>::Hash) {
        channel.update_digest(poseidon_hash_bls(channel.digest(), root));
    }
}
...
mod tests {
    ...
    // TODO: Redo test vectors for bls
    /*#[test]
    fn test_vector() {
        assert_eq!(
            PoseidonBLSMerkleHasher::hash_node(None, &[m31!(0), m31!(1)]),
            BlsFr::from_str(
    ...
    fn test_merkle_success() {
        let (queries, decommitment, values, verifier) = prepare_merkle::<PoseidonBLSMerkleHasher>();
    ...
    fn test_merkle_invalid_witness() {
        let (queries, mut decommitment, values, verifier) =
            prepare_merkle::<PoseidonBLSMerkleHasher>();
        decommitment.hash_witness[4] = BlsFr::default();
    ...
    fn test_merkle_witness_too_long() {
        let (queries, mut decommitment, values, verifier) =
            prepare_merkle::<PoseidonBLSMerkleHasher>();
        decommitment.hash_witness.push(BlsFr::default());
